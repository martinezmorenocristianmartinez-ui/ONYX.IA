...
* **Interfaz Holográfica:** Orbe reactivo y esquema de color inspirado en ONYX (Era de Ultrón) con tema dorado y animaciones de procesamiento dinámicas.
...
* **Programación Autónoma en Sandbox:** ONYX puede escribirse sus propios scripts de habilidades (`auto_programmer`), compilarlos en frío y ejecutarlos con un timeout seguro en un entorno de pruebas, para inyectar su propio código en tiempo real si tiene éxito.
...
2. Clona este repositorio en tu escritorio:
   ```bash
   git clone https://github.com/tu-usuario/ONYX-AI.git
   ```
3. Ejecuta el archivo `Instalar_ONYX.bat` para construir el entorno virtual (`.venv`) e instalar automáticamente todas las dependencias (`requirements.txt`).
4. Para iniciarlo rápidamente (y en segundo plano), puedes utilizar **`Iniciar ONYX Beta.vbs`**.
