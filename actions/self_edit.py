# -*- coding: utf-8 -*-
"""
self_edit.py — Permite a ONYX editar sus propios archivos de código fuente.
...
# Raíz del proyecto ONYX
ONYX_ROOT = Path(__file__).resolve().parent.parent
BACKUP_DIR = ONYX_ROOT / "backups"
...
def _make_backup(file_path: Path) -> str:
    ...
    relative = file_path.relative_to(ONYX_ROOT)
...
class PathEscapeError(Exception):
    """Se intentó acceder a una ruta fuera de la raíz del proyecto ONYX."""


def _resolve_path(file_ref: str) -> Path:
    """
    Resuelve una referencia de archivo de forma SEGURA.

    Solo se permiten rutas que queden DENTRO de ONYX_ROOT. Se rechazan rutas
    absolutas (ej: C:\\Windows\\...) y traversal con '..' que escapen del
    proyecto. Esto evita que ONYX se edite/sobrescriba archivos del sistema.
    """
    p = Path(file_ref)
    if p.is_absolute() or (len(file_ref) >= 2 and file_ref[1] == ":"):
        # Ruta absoluta o con unidad de Windows (C:\...) — prohibido.
        raise PathEscapeError(
            f"Ruta absoluta no permitida: '{file_ref}'. Usá rutas relativas al proyecto."
        )

    resolved = (ONYX_ROOT / p).resolve()
    root_resolved = ONYX_ROOT.resolve()

    # Verificar contención dentro de la raíz (Python 3.9+: is_relative_to).
    try:
        is_inside = resolved.is_relative_to(root_resolved)
    except AttributeError:  # pragma: no cover - Python < 3.9
        is_inside = str(resolved).startswith(str(root_resolved) + os.sep) or resolved == root_resolved

    if not is_inside:
        raise PathEscapeError(
            f"Ruta fuera del proyecto ONYX: '{file_ref}'. Acceso denegado por seguridad."
        )
    return resolved
...
def self_edit(parameters: dict, player=None) -> str:
    """
    Auto-edición de código de ONYX.
    ...
    # ── EDIT (buscar y reemplazar) ────────────────────────────────────────
        ...
        try:
            ...
            diff_str = "".join(diff[:100])  # Máximo 100 líneas de diff
            if len(diff) > 100:
                diff_str += "\n... [diff truncado]"
    ...
    # ── LIST BACKUPS ──────────────────────────────────────────────────────
        ...
        for b in backups[:50]:  # Mostrar máximo 50
            size_kb = b.stat().st_size / 1024
            lines.append(f"  - {b.name} ({size_kb:.1f} KB)")
        if len(backups) > 50:
            lines.append(f"  ... y {len(backups) - 50} más")
        return "\n".join(lines)
    ...
    # ── RESTORE BACKUP ────────────────────────────────────────────────────
        ...
        original_path = ONYX_ROOT / original_rel
