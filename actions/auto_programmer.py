...
# Solo se permiten nombres de herramienta que sean identificadores Python simples.
# Esto evita inyección de ruta (ej: tool_name='../../main' o nombres con / \ ..)
# que permitiría sobrescribir archivos fuera de actions/.
_SAFE_TOOL_NAME = re.compile(r"^[a-z_][a-z0-9_]{1,48}$")


def _is_safe_tool_name(name: str) -> bool:
    return bool(_SAFE_TOOL_NAME.match(name or "")) and not name.startswith("__")
...
"""
    try:
        # Asegurarse de que el PYTHONPATH incluya el directorio de ONYX
        import os
        env = os.environ.copy()
        ...
        return False, "Error: Timeout de ejecución en sandbox (límite de 120 segundos excedido)."
    except Exception as e:
...
def auto_programmer(parameters: dict, player=None) -> str:
    ...
    if not _is_safe_tool_name(tool_name):
        return (
            f"Error de seguridad: 'tool_name' inválido ('{tool_name}'). "
            "Debe ser un identificador simple (letras minúsculas, números y guiones bajos, "
            "sin barras, puntos ni '..'). Ej: 'mi_herramienta'."
        )

    actions_dir = Path(__file__).resolve().parent
    tool_file = actions_dir / f"{tool_name}.py"

    # Defensa en profundidad: el archivo resultante debe quedar dentro de actions/.
    if tool_file.resolve().parent != actions_dir.resolve():
        return "Error de seguridad: la ruta del archivo escapa de la carpeta actions/."
    ...
    if action == "create_tool" or action == "fix_tool":
        ...
        # 6. Forzar la reconexión de sesión cognitiva de ONYX
        reload_msg = ""
